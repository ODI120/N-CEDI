...
type Row = { id: string; name: string; role: string; content: string; image_url: string | null; sort_order: number; is_published: boolean; created_at: string }
...
const form = ref({ name: '', role: '', content: '', image_url: '', sort_order: 0, is_published: true })
const target = ref<Row | null>(null)

const openAdd = () => { mode.value = 'add'; form.value = { name: '', role: '', content: '', image_url: '', sort_order: 0, is_published: true }; modalOpen.value = true }
const openEdit = (r: Row) => { mode.value = 'edit'; target.value = r; form.value = { name: r.name, role: r.role, content: r.content, image_url: r.image_url || '', sort_order: r.sort_order, is_published: r.is_published }; modalOpen.value = true }
...
const save = async () => {
  ...
  try {
    if (mode.value === 'add') {
      const { error } = await supabase.from('testimonials').insert([form.value])
      if (error) throw error
      toast.add({ title: 'Testimonial created', color: 'green' })
    } else {
      const { error } = await supabase.from('testimonials').update(form.value).eq('id', target.value!.id)
      if (error) throw error
  ...
  <section class="admin-page">
    <div class="ap-header">
      <div class="ap-header__left">
        ...
        <p class="ap-subtitle">Manage student and partner success stories.</p>
      </div>
      <div class="ap-header__actions">
        <button class="btn btn-ghost" @click="refresh()"><UIcon name="i-lucide-refresh-cw" />Refresh</button>
        <button class="btn btn-primary" @click="openAdd"><UIcon name="i-lucide-plus" />Add Testimonial</button>
      </div>
    </div>
    ...
    <AdminTable :columns="columns" :rows="data || []" :loading="pending" empty-title="No testimonials">
    ...
    <AdminModal :open="modalOpen" :title="mode === 'add' ? 'New Testimonial' : 'Edit Testimonial'" :submit-label="mode === 'add' ? 'Create' : 'Save'" :loading="saving" @close="modalOpen = false" @submit="save">
      <div class="am-field"><label class="am-label">Name</label><input v-model="form.name" class="am-input" /></div>
      <div class="am-field"><label class="am-label">Role/Title</label><input v-model="form.role" class="am-input" /></div>
      ...
      <div class="am-field"><label class="am-label">Content</label><textarea v-model="form.content" class="am-textarea" rows="4" /></div>
      <div class="am-row-2">
        ...
        <div class="am-field" style="justify-content:flex-end"><label class="am-checkbox-row"><input type="checkbox" v-model="form.is_published" /> Published</label></div>
      </div>
    </AdminModal>
