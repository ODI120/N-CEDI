...
const openAdd = () => { mode.value = 'add'; form.value = { title: '', slug: '', excerpt: '', category_id: '', is_published: false }; slugTouched.value = false; modalOpen.value = true }
const openEdit = (r: Row) => { mode.value = 'edit'; target.value = r; form.value = { title: r.title, slug: r.slug, excerpt: r.excerpt || '', category_id: r.category_id || '', is_published: r.is_published }; slugTouched.value = true; modalOpen.value = true }
  ...
  <section class="admin-page">
    <div class="ap-header">
      ...
      <div class="ap-header__actions">
        ...
        <button class="btn btn-primary" @click="openAdd" v-if="canEdit"><UIcon name="i-lucide-plus" />Write Post</button>
      </div>
    </div>
    ...
    <AdminModal :open="modalOpen" :title="mode === 'add' ? 'New Post' : 'Edit Post'" :submit-label="mode === 'add' ? 'Create' : 'Save'" :loading="saving" @close="modalOpen = false" @submit="save">
      ...
      <div class="am-field"><label class="am-label">Slug</label><input v-model="form.slug" class="am-input" @input="slugTouched = true" /></div>
